...
(function () {
    ...
    CAAT.MenuContainer.prototype = {
		...
		var exportkeyButton = new CAAT.Button().initialize(director,sheetImage,0,1,2,0,function(){
			if(global.taothachdau) {
				var fid=prompt("dien fid (-1) de random","10001");

				if (fid!=null)
				  {
				  global.taothachdau(fid);
				  }
			}
		},
		function(){},
		...
		function(){self.hoverButton = -1;}).
			setLocation(buttonX,buttonYStart+buttonSize*3+buttonYDelta*6);
		self.exportkeyButton = exportkeyButton;
		self.addChild(exportkeyButton);

		var testchaButton = new CAAT.Button().initialize(director,sheetImage,0,1,2,0,function(){
			if(global.nhanthachdau) {

				var _id= global.challenge[0]._id;
				var song=global.challenge[0].song;
				var fid=prompt("dien fid (-1) de random",_id);

				if (fid!=null)
				  {
				  global.nhanthachdau(fid,song);
				  }
			}
		},
		function(){},
		function(){},
		function(){self.hoverButton = 9;},
		function(){self.hoverButton = -1;}).
			setLocation(buttonX,buttonYStart+buttonSize*4+buttonYDelta*8);
		self.testchaButton = testchaButton;
		self.addChild(testchaButton);
