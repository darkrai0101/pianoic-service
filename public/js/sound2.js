 ...
 CAAT.SoundPiano.prototype  = {
	...
	playMusic: function(soundId,canplayCallback,justLoad){
		...
		if(typeof audio == "undefined"){
			...
			//*/
			if (audioDescriptor[i].path.substr(audioDescriptor[i].path.lastIndexOf('.')+1)==='js'){
				var temp;
				var path =audioDescriptor[i].path;
				var first=path.lastIndexOf('/')+1;
		 		var last=path.lastIndexOf('.');
		 		var songname=path.substring(first,last);
				...
				//DOMLoader.script.add(audioDescriptor[i].path);
			 	$.getScript(path, function() {			 		
			 		...
			 		audio = new Audio( mimetype+ song);console.log(self.audioObjects)
